...
function gestisciForm(event) {
    ...
    if (codice !== "") {
            if (codiciSconto.includes(codice)){
                ...
                outputPrezzo.innerHTML = `<span id="titolo-p"><h5 class="fs-5 text-dark">Prezzo finale</h5><p><i class="fs-5 text-dark">&euro;</i><span class="fw-bold fs-5 text-dark"> ${parteIntera},</span><span class="fw-normal fs-6 text-secondary">,${decimali}</p></span>`;
                outputScontoApplicato.innerHTML = `<button type="button" class="btn btn-sm btn-success" id= "sconto-true">Sconto del 25 % applicato</button>`;
            } else {
                outputScontoApplicato.innerHTML = `<button type="button" class="btn btn-sm btn-danger" id="sconto-false">Codice non valido</button>`;
            }
    }

    /* Assegnazione output generale fuori dalla condizione */
    outputPrezzo.innerHTML = `<span id="titolo-p"><h5 class="fs-5 text-dark">Prezzo finale</h5><p><i class="fs-5 text-dark">&euro;</i><span class="fw-bold fs-5 text-dark"><span class="fw-bold fs-5 text-dark"> ${parteIntera}</span><span class="fw-normal fs-6 text-secondary">,${decimali}</p></span>`;
